...
def _render(slot, lines, status="RUNNING", error=False):
    body = "".join(f"<div class='scan-log-line'>{html.escape(line)}</div>" for line in lines[-10:])
    ...
    slot.markdown(
        ...
        .scan-activity-body{{padding:7px 9px;max-height:175px;overflow:hidden;font-size:9px;line-height:1.55}}
    ...
    def wrapped_run_scan(*args, **kwargs):
        ...
        def activity_callback(done, total, message):
            ...
            if msg != last_message["value"]:
                should_log = True
                # Avoid flooding the terminal with every batch update.
