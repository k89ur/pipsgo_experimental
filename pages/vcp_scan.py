...
def apply_vcp_result_filters(data):
    search = st.session_state.get("vcp_result_search", "")
    selected_indexes = st.session_state.get("vcp_result_index_filter", [])
    selected_industries = st.session_state.get("vcp_result_industry_filter", [])
...
def vcp_result_filter_controls(data):
    ...
    return apply_vcp_result_filters(data)
...
if st.session_state.vcp_full_table:
    ...
    if df is None or df.empty:
        ...
        filtered_df = vcp_result_filter_controls(df)
        full = prepare_table(filtered_df[[c for c in visible_vcp_columns() if c in filtered_df.columns]])
        full.insert(0, "S.No", range(1, len(full) + 1))
        st.markdown(f'<div class="section-title">VCP Results · {len(filtered_df):,}</div>', unsafe_allow_html=True)
...
with main:
    ...
    if df is not None:
        ...
        if df.empty:
            ...
            view_df = vcp_result_filter_controls(df)
