...
COLUMNS = [
    "Symbol", "Index", "Industry", "LTP", "VCP Score", "Contractions",
    "C1 %", "C2 %", "C3 %", "C4 %", "Final Contraction %", "Pivot",
    "From Pivot %", "52W High", "From 52W High %", "Avg Volume 50D",
    "Volume Contracting", "Trend OK", "Breakout", "TradingView"
]
...
if st.session_state.vcp_full_table:
    ...
    if df is None or df.empty:
        ...
        full = df[[c for c in COLUMNS if c in df.columns]].copy()
...
with main:
    with st.container(border=True):
        ...
        with b:
            ...
            final_max = st.slider("Maximum final contraction %", 1.0, 10.0, 5.0, step=0.5, key="vcp_final_max")
        with c:
            final_min = st.slider("Final contraction minimum %", 0.0, 5.0, 0.0, step=0.5, key="vcp_final_min")
        ...
        with e:
            near_high = st.slider("Within 52W high %", 1.0, 15.0, 7.0, step=0.5, key="vcp_near_high")
        with f:
        ...
        with h:
            near_pivot = st.slider("Near pivot %", 1.0, 15.0, 5.0, step=0.5, key="vcp_near_pivot")
        st.markdown('<div style="height:.15rem"></div>', unsafe_allow_html=True)
        ...
        with scan:
            if st.button("⛶ Full Results", use_container_width=True, key="vcp_full"):
    ...
    try:
        ...
        with st.spinner("Running VCP scan…"):
            result, stats = vcp_engine.run_scan(
                min_contractions=min_c, max_contractions=max_c,
                first_max=first_max, final_max=final_max,
                final_min_tightness=final_min,
                volume_required=volume_required,
                near_high_pct=near_high, min_price=min_price,
                min_avg_volume=min_avg_volume, trend_filter=trend_filter,
                near_pivot_pct=near_pivot,
                breakout_already_occurred=False,
                quality=quality, batch_size=vcp_engine.DEFAULT_BATCH_SIZE,
                snapshot_mode=mode, progress_callback=update,
            )
...
with main:
    ...
    if df is not None:
        ...
        if df.empty:
            ...
            if len(df) > 50:
                st.caption(f"Showing top 50 of {len(df):,} matches. Use Full Results for the complete table.")
