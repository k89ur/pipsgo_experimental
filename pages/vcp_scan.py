...
with main:
    with st.container(border=True):
        ...
        with k: final_age_max=st.slider("Final contraction started within (trading days)",5,60,20,step=1,key="vcp_final_age")
        with l: st.caption("Active VCP filter: the latest qualifying contraction must have started within this many trading bars.")
        st.markdown('<div style="height:.15rem"></div>',unsafe_allow_html=True)
...
if live or eod:
    ...
    try:
        ...
        with st.spinner("Running VCP scan…"):
            result,stats=vcp_engine.run_scan(min_contractions=min_c,max_contractions=max_c,first_max=first_max,final_max=final_max,final_min_tightness=final_min,volume_required=volume_required,near_high_pct=near_high,min_price=min_price,min_avg_volume=min_avg_volume,trend_filter=trend_filter,near_pivot_pct=near_pivot,breakout_already_occurred=breakout,quality=quality,final_contraction_max_age=final_age_max,batch_size=vcp_engine.DEFAULT_BATCH_SIZE,snapshot_mode=mode,progress_callback=update)
