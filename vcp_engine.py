...
QUALITY_PRESETS = {
    "Strict": {"swing_window": 4, "progress_tolerance": .10, "volume_tolerance": .05},
    "Standard": {"swing_window": 4, "progress_tolerance": .18, "volume_tolerance": .10},
    "Loose": {"swing_window": 3, "progress_tolerance": .28, "volume_tolerance": .18},
}
...
def _extrema(x, window):
    hi = x.High.rolling(window * 2 + 1, center=True).max()
    lo = x.Low.rolling(window * 2 + 1, center=True).min()
    pts = []
    ...
    for p in pts:
        if not out or out[-1][1] != p[1]:
            ...
            old = out[-1]
            out[-1] = p if ((p[1] == "H" and p[2] >= old[2]) or (p[1] == "L" and p[2] <= old[2])) else old
    return out
...
def _candidate_contractions(x, window, lookback=180):
    ...
    pts = _extrema(y, window)
    out = []
    for j in range(len(pts) - 2):
        p1, p2, p3 = pts[j:j + 3]
        if p1[1] != "H" or p2[1] != "L" or p3[1] != "H" or not (p1[0] < p2[0] < p3[0]):
            continue
    ...
    for end in range(len(candidates)):
        for start in range(max(0, end - 6), end + 1):
            ...
            if any(depths[k + 1] > depths[k] * (1 + tolerance) for k in range(len(depths) - 1)):
                continue
            if any(seq[k + 1]["recovery_high"] < seq[k]["recovery_high"] * (.75 - tolerance) for k in range(len(seq) - 1)):
                continue
...
def analyze_vcp(symbol, frame, min_contractions=2, max_contractions=4, first_max=25., final_max=5., final_min_tightness=0., volume_required=True, near_high_pct=7., min_price=100., min_avg_volume=0., trend_filter=True, near_pivot_pct=5., breakout_already_occurred=False, quality="Standard"):
    ...
    if len(x) < 253:
        return {}
    p = QUALITY_PRESETS.get(quality, QUALITY_PRESETS["Standard"])
    seq = _choose_sequence(_candidate_contractions(x, p["swing_window"]), min_contractions, max_contractions, first_max, final_max, p["progress_tolerance"])
    ...
    if seq:
        ...
        # The highest recovery level in the detected sequence is the active pivot.
        pivot = max(c["recovery_high"] for c in seq)
        pdist = (pivot - close) / pivot * 100 if pivot else np.nan
        final = depths[-1]
        volcontract = all(vols[i + 1] <= vols[i] * (1 + p["volume_tolerance"]) for i in range(len(vols) - 1))
        ...
        breakout_status = "Breakout" if breakout else ("Near Pivot" if np.isfinite(pdist) and pdist >= 0 and pdist <= near_pivot_pct else "Below Pivot")
    else:
        depths = []
    ...
    return {
        ...
        "Final Contraction %": final,
        "Pivot": pivot,
        "From Pivot %": pdist,
        "52W High": prior,
        "From 52W High %": from52,
        "Avg Volume 50D": avgvol,
        "Volume Contracting": volcontract,
        "Trend OK": trend,
        "Breakout": breakout,
        "Breakout Status": breakout_status,
        "Breakout Contractions": len(seq) if breakout else np.nan,
        "VCP Score": score,
        "History Days": len(x),
    }
