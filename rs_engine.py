...
def run_scan(min_rs: int = 80, near_high_pct: float = 5, min_price: float = 100, rising_days: int = 20,
    ...
    for start in range(0, total, batch_size):
        ...
        data = _download_batch(batch); successful += len(data)
        ...
        if progress_callback: progress_callback(done, total, "Downloading & calculating")
        time.sleep(0.05)
    ...
    # Enrich only the final matches so sector lookup does not slow the full scan.
    if not df.empty:
        df["Sector"] = [_sector_for_symbol(s) for s in df["Symbol"].astype(str)]
