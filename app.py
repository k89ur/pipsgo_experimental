...
.hero-sub { color:var(--muted); font-size:.86rem; margin-top:.45rem; }
.asof { color:var(--muted); font-size:.78rem; text-align:right; }
.asof strong { color:var(--text); font-size:.9rem; display:block; margin-top:.18rem; }
...
.stat-value { font-size:1.25rem; font-weight:650; margin-top:.25rem; }

.section-title { font-size:.76rem; color:var(--muted); text-transform:uppercase; letter-spacing:.1em; font-weight:600; margin:1.8rem 0 .65rem; }
.leader-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:.7rem; }
.leader { background:var(--panel); border:1px solid var(--line); border-radius:11px; padding:.85rem 1rem; }
.leader-top { display:flex; justify-content:space-between; align-items:center; }
.leader-rank { color:var(--muted); font-size:.72rem; font-weight:600; }
.leader-name { font-size:.92rem; font-weight:600; margin-top:.35rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.leader-score { font-size:1.55rem; font-weight:700; }
.score-strong { color:var(--green); }
.score-mid { color:var(--amber); }
.score-weak { color:var(--red); }

.toolbar { display:flex; gap:.65rem; align-items:center; margin:.7rem 0 .8rem; }
...
.legend { color:var(--muted); font-size:.72rem; margin:.55rem 0 1.2rem; }
.dot { display:inline-block; width:7px; height:7px; border-radius:50%; margin:0 .25rem 0 .7rem; }
.dot:first-child { margin-left:0; }

.footer { color:#606a79; font-size:.72rem; border-top:1px solid var(--line); padding-top:1rem; margin-top:1.5rem; }

@media (max-width: 700px) {
    .block-container { padding:1.25rem .65rem 2.5rem; }
...
<div class="hero">
  ...
  <div class="asof">LATEST DATA<strong>—</strong></div>
</div>
""", unsafe_allow_html=True)
...
# Re-render the hero's date once data exists.
...
with l1:
    st.download_button("↓  Export CSV", df.to_csv(index=False).encode("utf-8"), "nifty_index_rs.csv", "text/csv", use_container_width=True)
with l2:
